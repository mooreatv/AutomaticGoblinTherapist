...
AGTToShortResponse =	{
						[1] = {Text = "OK... \"%s\". Tell me more.", Selected = false,},
						}
			...
			if GetTime() > AGTOptionsSettings["Global"].RemoveInaktiveAfter.Value then
				for x = 1, 5, 1 do
					if _G["AGTMainTherapy"..x.."FS"]:GetText() ~= "Empty Seat" then
						if AGTPatients[_G["AGTMainTherapy"..x.."FS"]:GetText()].LastContact < (GetTime()-AGTOptionsSettings["Global"].RemoveInaktiveAfter.Value) then
...
function AGTSendResponse(GoblinsResponse, patient)
	...
	table.insert(AGTSendQueue, {["sendat"] = (math.floor(GetTime()) + ((string.len(GoblinsResponse)/AGTOptionsSettings["Global"].CharsPerMinute.Value) * 60)), ["msg"] = GoblinsResponse, ["to"] = patient})
...
function AGTRemoveSeat(Patient)
	for x = 1, 5, 1 do
		if _G["AGTMainTherapy"..x.."FS"]:GetText() == Patient and _G["AGTMainTherapy"..x.."FS"]:GetText() ~= "Empty Seat" then
	...
	if button == "LeftButton" then
		if _G["AGTMainTherapy"..x.."FS"]:GetText() ~= "Empty Seat" then
			if self.InTreatment == false then
				...
				AGTDialog(AGTPatients[_G["AGTMainTherapy"..x.."FS"]:GetText()].PrevMsg, _G["AGTMainTherapy"..x.."FS"]:GetText()) 
			else
...
f:SetWidth(260)  
f:SetHeight(145) 
f:SetScript("OnDragStart", function() _G["AGTMain"]:StartMoving() end)
f:SetScript("OnDragStop", function() _G["AGTMain"]:StopMovingOrSizing() end)
...
for x = 1, 5, 1 do
	...
	tf:SetScript("OnClick", function(self, button, down) AGTTherapyOnClick(self, button, down) end)
	tf:SetBackdrop({bgFile="", edgeFile="Interface\\Tooltips\\UI-Tooltip-Border", tile = false, tileSize = 0, edgeSize = 16, insets = { left = 0, right = 0, top = 0, bottom = 0 }})
...
function AGTOptionsShowOptionsHelp(framename, inout)
	if AGTOptionsSettings["Global"].AGTShowHelpTooltips.Value == AGTOptionsCONSTChecked	then
		...
		if AGTOptionsSettingsTemplate[AGTOptionsActualOptionsTemplate][string.sub(framename, string.find(framename, "Settings")+8)][tLocale] ~= "" then
			if inout == true then
					AGTOptionsTooltip:ClearLines() 
					AGTOptionsTooltip:SetOwner(_G[framename], "ANCHOR_TOPLEFT") 
					AGTOptionsTooltip:AddLine(AGTOptionsSettingsTemplate[AGTOptionsActualOptionsTemplate][string.sub(framename, string.find(framename, "Settings")+8)][tLocale], 1, 1, 1, 1) 
...
function AGTOptionsLoadSettings()
	...
	table.sort(tempSorttable, 	function(a, b) 
									if AGTOptionsSettingsTemplate[AGTOptionsActualOptionsTemplate][a]["Sort"] < AGTOptionsSettingsTemplate[AGTOptionsActualOptionsTemplate][b]["Sort"] then
										return true 
									else
										return false 
									end
	...
	table.foreach(tempSorttable, 
		function(key, value)						
			...
			table.foreach(AGTOptionsSettingsTemplate[AGTOptionsActualOptionsTemplate][value], 
			...
			if value then	
				...
				if ttemplate == "AGTOptionsCheckTemplate" or ttemplate == "AGTOptionsColorPickerTemplate" or ttemplate == "AGTOptionsClickButtonTemplate" then
...
function AGTOptionsSaveSettings()
	...
	for _,child in ipairs(kids) do
		if child:IsVisible() then 
			if AGTOptionsSettingsTemplate[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Type == "Button" and AGTOptionsSettingsTemplate[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Template == "AGTOptionsColorPickerTemplate" then
				local r, g, b = child:GetBackdropColor() 
				AGTOptionsSettings[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Value.r = r 
				AGTOptionsSettings[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Value.g = g 
				AGTOptionsSettings[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Value.b = b 
			end
			if AGTOptionsSettingsTemplate[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Type == "Slider" then
				if AGTOptionsSettingsTemplate[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Decimal then
					AGTOptionsSettings[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Value = child:GetValue()
				else
					AGTOptionsSettings[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Value = math.floor(child:GetValue())
				end
			end
			if AGTOptionsSettingsTemplate[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Type == "CheckButton" then
				if child:GetChecked() == true then
					AGTOptionsSettings[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Value = AGTOptionsCONSTChecked 
				end
				if child:GetChecked() == false then
					AGTOptionsSettings[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Value = AGTOptionsCONSTUnChecked 
				end
			end
			if AGTOptionsSettingsTemplate[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Type == "EditBox" then
				AGTOptionsSettings[AGTOptionsActualOptionsTemplate][string.sub(child:GetName(), string.find(child:GetName(), "Settings")+8)].Value = child:GetText() 
			end
		end
	end
	AGTUpdateVisuals()
...
function AGTOptionsInitAll()
	...
	for x = 1, AGTOptionsNoOptionsFrameTemplates, 1 do
		...
		table.foreach(AGTOptionsSettingsTemplate[AGTOptionsOptionsTemplate[x]], 
			function(key, value)
		...
		for x = 1, AGTOptionsNoOptionsFrameTemplates, 1 do
			if AGTOptionsSettings[AGTOptionsOptionsTemplate[x]] then
				table.foreach(AGTOptionsSettings[AGTOptionsOptionsTemplate[x]], 
					function(key, value)
						if AGTOptionsSettingsTemplate[AGTOptionsOptionsTemplate[x]][key] == nil then
							DEFAULT_CHAT_FRAME:AddMessage("Old setting "..key.." in "..AGTOptionsOptionsTemplate[x].." not longer in use - Removed.") 
		...
		if _G["AGTOptionsTabs"..AGTOptionsOptionsTemplate[x]] then
			...
			tframe = CreateFrame("Button", "AGTOptionsTabs"..AGTOptionsOptionsTemplate[x], _G["AGTOptionsTabs"], "UIPanelButtonTemplate") 
		end
		if x == 7 then 
			y = y - 22 
			xs = (((x-1)*75)) 
...
AGTOptionsSettingsTemplate["Global"] = 	
	...
	Enabled =	{
					Sort = 1,
					Type = "CheckButton", Value = AGTOptionsCONSTChecked, Template = "AGTOptionsCheckTemplate",
					OnClick = 	function() 
									AGTOptionsSaveSettings()
								end,
	...
	HideIfEmpty =	{
					Sort = 1.5,
					Type = "CheckButton", Value = AGTOptionsCONSTUnChecked, Template = "AGTOptionsCheckTemplate",
					OnClick = 	function() 
									AGTOptionsSaveSettings()
								end,
	...
	RemoveInaktiveAfter  = {
					Sort = 2,
					Type = "Slider", Value = 300, Min = 10, Max = 600, Step = 10, Template = "AGTOptionsSliderTemplate",
	...
	CharsPerMinute  = {
					Sort = 3,
					Type = "Slider", Value = 400, Min = 100, Max = 600, Step = 10, Template = "AGTOptionsSliderTemplate",
					OnValueChanged = function(self) 
										if AGTOptionsCONSTSaveValue == true then 
											AGTOptionsSaveSettings() 
										end
	...
	Quotes = {
					Sort = 3.6,
					Type = "Slider", Value = 0, Min = 0, Max = 100, Step = 10, Template = "AGTOptionsSliderTemplate",
					OnValueChanged = function(self) 
										if AGTOptionsCONSTSaveValue == true then 
											AGTOptionsSaveSettings() 
										end
	...
	EditLib = 		{
					Sort = 7.5,
					Type = "Button", Value = "Edit Library", Min = 0, Max = 0, Template = "AGTOptionsClickButtonTemplate",
					OnClick = 	function()  
											AGTOpenLib() 
								end,
	...
	Reset = 		{
					Sort = 8,
					Type = "Button", Value = "Reset Settings", Min = 0, Max = 0, Template = "AGTOptionsClickButtonTemplate",
					OnClick = 	function()  
									AGTOptionsReset() 
								end,
...
AGTOptionsSettingsTemplate["Visuals"] = 	
	...
	Scale  = {
					Sort = 1,
					Type = "Slider", Value = 1, Min = 0.3, Max = 1.5, Step = 0.1, Template = "AGTOptionsSliderTemplate", Decimal = true,
					OnValueChanged = function(self) 
										if AGTOptionsCONSTSaveValue == true then 
											AGTOptionsSaveSettings() 
										end
	...
	NumberSeats  = {
					Sort = 2,
					Type = "Slider", Value = 3, Min = 1, Max = 5, Step = 1, Template = "AGTOptionsSliderTemplate",
					OnValueChanged = function(self) 
										if AGTOptionsCONSTSaveValue == true then 
											AGTOptionsSaveSettings() 
										end
	...
	FontType  = {
					Sort = 4,
					Type = "Slider", Value = 3, Min = 1, Max = 26, Step = 1, Template = "AGTOptionsSliderTemplate",
					OnValueChanged = function(self) 
										if AGTOptionsCONSTSaveValue == true then 
											AGTOptionsSaveSettings() 
										end
...
AGTOptionsSettingsTemplate["Sounds"] = 	
	...
	WhisperInSurgery  = {
					Sort = 1,
					Type = "Slider", Value = 1, Min = 1, Max = 16, Step = 1, Template = "AGTOptionsSliderTemplate",
					OnValueChanged = function(self) 
										if AGTOptionsCONSTSaveValue == true then 
											AGTOptionsSaveSettings() 
										end
	...
	Ignore  = {
					Sort = 3,
					Type = "Slider", Value = 11, Min = 1, Max = 16, Step = 1, Template = "AGTOptionsSliderTemplate",
					OnValueChanged = function(self) 
										if AGTOptionsCONSTSaveValue == true then 
											AGTOptionsSaveSettings() 
										end
			...
			if string.find(self:GetName(), "AGTEdit1") then
				if AGTkeyword[FauxScrollFrame_GetOffset(AGTEditScrollBar1) + self:GetID()].Selected == true and AGTParent == 1 then
					...
					for x = 1, table.getn(AGTkeyword[FauxScrollFrame_GetOffset(AGTEditScrollBar1) + self:GetID()].Responses), 1 do
						AGTresponse[AGTkeyword[FauxScrollFrame_GetOffset(AGTEditScrollBar1) + self:GetID()].Responses[x]].Selected = true
					end
				end
			elseif string.find(self:GetName(), "AGTEdit2") then
...
function AGTNewKeyButton()
	if AGTEditEditboxText3:GetText() ~= "" then
		...
		if notnew > 0 then
			...
			table.insert(AGTkeyword, {key = string.upper(AGTEditEditboxText3:GetText()), Responses = {}, Selected = false , Q = false})
			AGTEditScrollBars_Update()
...
function AGTOptionsOutputDropDown_OnLoad(frame, source, value)
	...
	UIDropDownMenu_Initialize(frame, function() 
										local entry = { func = function(self) AGTOptionsOutputDropDown_OnClick(self, tname) end }
